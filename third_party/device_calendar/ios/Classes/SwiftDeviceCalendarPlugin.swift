...
public class SwiftDeviceCalendarPlugin: NSObject, FlutterPlugin {
    ...
    private func hasEventPermissions() -> Bool {
        ...
        // iOS 17+ reports granted access as `.fullAccess`, not the legacy
        // `.authorized` case — checking only `.authorized` makes every
        // permission-gated call (retrieveCalendars, retrieveEvents, ...)
        // fail as "unauthorized" even though access was actually granted.
        // (Patched from upstream device_calendar 3.9.0, which only checks
        // `.authorized`; see third_party/device_calendar/README_PATCH.md.)
